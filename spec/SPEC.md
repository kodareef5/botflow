...
- a **relative path** from the referencing board's root, resolving to a board root as: if `<path>/board.yaml` exists, `<path>` is the root; else if `<path>/.botflow/board.yaml` exists, `<path>/.botflow` is the root; else the reference is dangling (lint `board-path-missing`). The path MUST stay inside the workspace: for the repo convention `<repo>/.botflow`, the workspace boundary is `<repo>` (so `../worker` is valid); for a bare board, it is the board root itself. An absolute path, or one whose `..` segments climb above that boundary, is lint error `board-path-escape`, and tools MUST refuse to write such a value onto a card;
...
- Promoting a checklist item creates a normal card, checks the source item, and writes inverse `parent`/`subtask` relations in the same prevalidated mutation. The promoted card inherits labels, accountable/delegated actors, due date, and estimate unless the caller overrides them.
...
- Hosted interactive activity and comment histories MUST be returned in bounded server-side pages, newest entry first. Filtering to a card or other scope happens before the page limit. A cursor over an append-only card section MUST remain stable when newer entries are appended, so paging toward older entries neither skips nor duplicates existing entries.
...
rules execute in definition order as part of the same prevalidated mutation. Every filter is
evaluated against one post-primary, pre-rule snapshot; actions from an earlier rule do
not widen or narrow a later rule's target. More than 16 is
